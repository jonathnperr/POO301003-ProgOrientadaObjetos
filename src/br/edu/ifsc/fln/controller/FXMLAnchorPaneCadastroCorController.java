...
public class FXMLAnchorPaneCadastroCorController implements Initializable {
    ...
    @FXML
    private Label lbIdCor;

    @FXML
    private Label lbNomeCor;
    
    @FXML
    private TableColumn<Cor, String> tcCadastroCor;

    @FXML
    private TableView<Cor> tvCadastroCor;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        tvCadastroCor.getSelectionModel().selectedItemProperty().
            addListener((observable, oldValue, newValue) -> selecionarItemTableViewCor(newValue));
    }    
    
    //metodos pra carregar a tabela e seleçao 
    public void carregarTableViewCor() {
        tcCadastroCor.setCellValueFactory(new PropertyValueFactory<>("nome"));
        listaCor = corDAO.listar();
        
        observableListCor = FXCollections.observableArrayList(listaCor);
        tvCadastroCor.setItems(observableListCor);
    }
    
    public void selecionarItemTableViewCor(Cor cor) {
        if (cor != null) {
            lbIdCor.setText(String.valueOf(cor.getId())); 
            lbNomeCor.setText(cor.getNome());
        } else {
            lbIdCor.setText(""); 
            lbNomeCor.setText("");
        }
    ...
    @FXML
    void handleBtAlterar(ActionEvent event) throws IOException { //tirar???
        Cor cor = tvCadastroCor.getSelectionModel().getSelectedItem();
        if (cor != null) {
